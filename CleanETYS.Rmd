...
Generateion
```{r}
Substation %>% filter(grepl("Brigg",Site.Name, ignore.case = TRUE))

SubGraph("DRAX")

```

solving the simultaneos equations of the impedance matrix to get voltage and current
```{r}
...
#slice is not working becuase the data is stacked!

Impedance %>% select(Node1, Node2, B2) %>% slice(c(1976,4385)) #self loop

Impedance %>% select(Node1, Node2, B2) %>% .[c(1990, 4399),]

dups<- test[duplicated(paste0(test$Edgename, test$Node1))|duplicated(paste0(test$Edgename, test$Node1), fromLast = T),]

checkdup <- Impedance %>% filter(Node1 == "CRAI1-")
