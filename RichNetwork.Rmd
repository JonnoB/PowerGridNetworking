...
RandomAttack <-function(graph = g, Nodes = 1, seed = NULL){
  #This is a simple attack strategy that can be used as an arguement in the "AttackTheGrid" function, it should be made
  #quosure before deploying it as an argument using quo(RandomAttack)
  ...
  deletevertex <- sample(V(graph)$name, Nodes)
  print(deletevertex)
...
AttackStrategy <-quo(RandomAttack)               
...
# RandomNodeAttack <-function(g, Nodes = 1){
#   sample(V(g)$name, Nodes)
# }
...
test <- list(gbase) %>%
  list %>%
  AttackTheGrid(., FixedStrategyAttack, MinMaxComp = 0.8, DeletionOrder = DeleteNodes, Target = "Nodes")

set.seed(15)
testEdges <- list(gbase) %>%
  list %>%
  AttackTheGrid(., FixedStrategyAttack, MinMaxComp = 0.8, DeletionOrder = DeleteEdges, Target = "Edges")




AdaptiveStrategyAttack <- function(g, AttackStrategy, Target = "Nodes", Number = 1, seed = NULL){
  ...
  deletevertex <- quo(UQ(AttackStrategy)(g, Target, Number)) %>%
    eval_tidy 
...
S1 <- quo(AdaptiveStrategyAttack(g, RandomAttack2))

S2<- quo(RandomAttack)
S2 <- quo(AdaptiveStrategyAttack())


set.seed(15)
test <- list(gbase) %>%
  list %>%
  AttackTheGrid(., S1, MinMaxComp = 0.8)



AdaptiveStrategyAttack <- function(g, AttackStrategy, Nodes = 1, seed = NULL){

    DeleteVect <- quo(UQ(AttackStrategy)(g)) %>%
    eval_tidy 

  #Stops crash caused by removing more nodes than there are... is this possible?
  if(vcount(g) > DeleteVect ){
    deletevertex <- DeleteVect[1:length(DeleteVect)]
  } else {
    
    deletevertex <- DeleteVect[1] 
  }
  
  print(deletevertex)
  #remove selected node from network
  g2 <- delete.vertices(g, deletevertex)
  return(g2)
