...
for(n in 1:100){
  ...
  GridList <- suppressMessages(AttackTheGrid(list(list(gbase)), FixedNodes, MinMaxComp = 0.005, CascadeMode =  c(TRUE,FALSE)[.x])) %>%
    map_df(~{
      #find the last element of the list which is the final network for that Cascade
      g <- .x[[length(.x)]] 
