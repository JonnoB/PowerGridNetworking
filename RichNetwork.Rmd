...
VertexMetaData <- trans1 %>%
  select(Bus.Name, 
         Voltage, 
         Demand, 
         Generation = Generation.B.....Peak.Security...Transport.Model., 
...
RandomAttack <-quote(sample(V(g)$name, 1))               
...
test <- list(gbase) %>%
  list %>%
  AttackTheGrid(., RandomAttack, MinMaxComp = 0.8, StopPoint = 15)
...
gCasc <- delete.vertices(test[[4]][[1]], "NECU10")
