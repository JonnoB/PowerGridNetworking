...
With the information above held in the graph each node would contain all necessary information for the analysis and would simplify a lot of the processesMelibe Viridis,
...
Things to do
create 1 node cascade check
node pair cascade check
...
test2 <- testattack  %>%
    map_df(~{
      ...
      .x[[length(.x)]] %>%
       ...
       data.frame(MaxComp = components(.)$csize %>% max,
                  TotalNodes = vcount(.),
                  TotalEdges = ecount(.)
                  )
    }
    ) %>%
  mutate(NodesRemoved = vcount(gbase) -0:(n()-1),
         GCfract = MaxComp/vcount(gbase),
         Simulation = n)
...
class(test)
        #extract maximum component size
       data.frame(MaxComp = components(test)$csize %>% max,
                  TotalNodes = vcount(test),
                  TotalEdges = ecount(test)
                  )
...
```{r}
#create an output statistic for the grid to look at it's shape and stuff
# The below doesn' twork due to not searching in the right environment
# set.seed(123)
# Bigdf <- 1:30 %>% map_df(~{
#   
#   print(paste("Simulation", .x))
# GridList <- suppressMessages(AttackTheGrid(list(list(gbase)), FixedNodes, MinMaxComp = 0.10, CascadeMode = FALSE)) %>%
#   unlist(., recursive = FALSE)
# 
#  GridList2<- GridList %>% map_dbl(~{
#  components(.x)$csize %>% max
# }
# ) %>% tibble(MaxComp = .) %>%
#   mutate(Nodes = vcount(gbase) -0:(n()-1),
#          GCfract = MaxComp/vcount(gbase),
#          Simulation = .x)
# 
# return(GridList2)
...
for(n in 1:100){
  ...
  mutate(NodesRemaining = vcount(gbase) -0:(n()-1),
         GCfract = MaxComp/vcount(gbase),
         Simulation = n)
...
saveRDS(Nodesims, "100simulationsRandomAttackto0015Nodes.rds")
...
saveRDS(Edgesims,"100simulationsRandomAttackto0015Edges.rds")
