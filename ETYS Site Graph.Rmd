...
sum(unique(GenerationData$Node.1) %in% FullnameCounts$Site)/length(unique(GenerationData$Node.1) )
...
#Terrible
sum(unique(LocalAssetData$Bus.Name) %in% FullnameCounts$Site)/length(unique(LocalAssetData$Bus.Name) )
...
None of the above works well so the networks need to be subsetted and explored more carefully
```{r}
 
NewNodes <- KNearestNodes(SubStatGraph, "ABNE", 2)

CompInject2 <- BalencedGenDem(SubStatGraph, InjectionVector, NewNodes)



 Impedance2 <- Impedance %>% 
  filter( Node1b %in% NewNodes & Node2b %in% NewNodes )
...
Admat[!is.finite(Admat)] <- 0 #Non finite numbers created by dividing by 0
 ...
 Transmission <- Impedance2 %>% 
  select(Node1b, Node2b, B) %>%
  group_by(Node1b, Node2b) %>%
  summarise(B2 = max(B, na.rm = TRUE))  %>%
  CreateTransmission(., "Node1b", "Node2b", "B2")
...
Admat2 <- as.integer(round(Admat,5)*10000) %>% matrix(nrow = 12)
Admat2b <-Admat2
diag(Admat2b)<-0
diag(Admat2)<- -rowSums(Admat2b)
rowSums(Admat2)

Admat2 <- Admat2
solve(Admat2, tol=1e-20)

test <- data_frame(sums = rowSums(Admat0), diag =  diag(Admat)) %>%
  mutate(ratio = sums/diag,
         diff = sums + diag)
```
