...
matpower_loader <- function(file_path, output_graph = TRUE){
  ...
  mat_list <- c(which(grepl("mpc.bus ", raw_mat$value)),
    ...
    purrr::map(~{
      ...
      raw_data <- raw_mat %>%
        dplyr::filter(1:nrow(.) > .x) %>% #filter all before and including the line declaring the network part data
        dplyr::filter(1:nrow(.) < which.max(grepl("];", value))) %>% #filter all after the final line of the data
        dplyr::mutate(value = sub(";", "", value) %>% gsub("%.*", "", .) %>% trimws()) %>%
        tidyr::separate(data = ., col = value, into = headers, sep = "\\t", convert = TRUE)
  ...
  bus_data <- dplyr::left_join(mat_list[[1]], mat_list[[2]], by = c("bus_i"="bus")) %>%
    dplyr::mutate_all(~ifelse(is.na(.), 0, .)) #non generator nodes have NAs these need to be replaced with 0's

  branch_data <- mat_list[[3]]

  if(output_graph){

    Out <- igraph::graph_from_data_frame(d = branch_data, directed = FALSE, vertices = bus_data)
