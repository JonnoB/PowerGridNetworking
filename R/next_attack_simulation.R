...
#'   the next attack simulation. This function is mostly only used as part of the multi_attack function
#' @param sim_order The output dataframe of multi_attack_order
#' @param folder The folder where the attack simulations are saved
#' @export
#' @seealso \code{\link{multi_attack_order}}, \code{\link{multi_attack}}
#' @examples
#'   next_attack_simulation(sim_order, folder)
#'
next_attack_simulation <- function(sim_order, folder){
CurrentSims <- list.files(folder) %>%
  str_replace_all(., "\\D", "" )
NeededSims <- sim_order$SimulationID %>%
