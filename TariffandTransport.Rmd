...
#why is the test and the impedance data frames nnot equal?
...
test<- Cascade(list(gbase),trans1, trans2)
...
coords1 <-layout_with_fr(gbase)


CoordLims <- coords1 %>% as_tibble %>%
  rename(x= V1, y = V2) %>% 
  gather %>%
  group_by(key) %>%
  summarise(max = min(value)/100,
            min = max(value)/100) %>%
  mutate(max = max*1.05,
         min = min*1.05) %>%
  ungroup

test <- coords1 %>% as_tibble %>% bind_cols(data_frame(Bus.Name = names(V(gbase))))
test2 <- data_frame(Bus.Name = names(V(FinalGrid))) %>%
  left_join(test, by= "Bus.Name")
...
gbase%>% 
  simplify(., remove.multiple = T, remove.loops = T) %>% 
  plot(.,
       layout = as.matrix(test[,1:2]),
       vertex.size=.8, 
       edge.arrow.size=.4, 
       vertex.label = NA#,
      # xlim = (CoordLims[1,2:3] %>% as.matrix),
      # ylim = (CoordLims[2,2:3] %>% as.matrix)
      )
...
im.convert(c("test1.png", "test2.png"), output = "test.gif")

1:length(test) %>% map(~{
 
  networkfile <- paste0("test", .x, ".png")
  print(networkfile)
...
im.convert(paste0("test", 1:4, ".png"), output = "test.gif")
```
