...
#this needs to work for multiple islands not just 1
#remove Slack bus, usually the largest generator
SlackRefCasc <- which.min(ValidNodesCasc$Bus.Order)

#error for wrong length
#Get new power flow across the new graph
...
#The giant componant condition, the process contrinues whilst this is true
...
#Using a weighted graph where weight is the Imp
```{r}
#input networkas_adjacency_matrix(Impmat, attr = "weight")
gCasc <-  Impedance %>%
  rename(weight = Y) %>%
